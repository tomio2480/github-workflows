...
`{md,markdown}` のような brace 記法は展開する．
解釈しきれない記法では絞り込みをやめ，変更ファイルをすべて選ぶ．
...
競合に負けた場合は相手の公開を待ち，完了したらその成果へ相乗りする．
待ち時間の上限は 60 秒で，`LINT_DEPS_PUBLISH_WAIT` で変更できる．
上限まで待っても現れないときは公開を諦め，自分の組み立て先をそのまま使う．
待ち続けて止まるより，1 度分の導入を無駄にするほうを選ぶ．
