...
`MD036`（no-emphasis-as-heading）は見出し代わりの太字を検出する．
日本語の技術文書では，見出しを立てるほどではない語の説明に
太字の小見出し（ラベル）を使う書き方が広く見られる．
中央で無効化するとラベルを使わない caller の検出力まで下がるため，
規則は有効のまま維持し，書き方の工夫で共存する
（[Issue #103](https://github.com/tomio2480/github-workflows/issues/103)）．

### 発火条件

`MD036` の発火条件は限定的である（`markdownlint` v0.38.0 実測）．

表 4: MD036 の発火条件（`markdownlint` v0.38.0 実測）

| 書き方 | 検出 |
|---|---|
| 太字だけの 1 行段落（句読点で終わらない） | 検出する |
| `**半値角** 正面の光度に対して…`（同一行に本文が続く） | 検出しない |
| `**半値角**` の直後の行に本文（空行なし・同一段落） | 検出しない |
| 複数行がすべて太字の段落 | 検出しない |
| 句読点（`punctuation` の記号）で終わる太字段落 | 検出しない |

### 推奨する書き方

太字の小見出しを使うときは，段落を分離しない次のどちらかで書く．

```markdown
**半値角** 正面の光度に対して半分の明るさになる角度である．

**半値角**
正面の光度に対して半分の明るさになる角度である．
```

太字だけの行を空行で独立させた場合，見出し代わりと判定される．
どうしても空行で分離したい箇所は，直前の行にコメントを置いて 1 段落だけ許容する．
使う指示は `<!-- markdownlint-disable-next-line MD036 -->` である．
複数箇所をまとめて許容する場合は `disable` と `enable` の対で範囲を閉じる．
`disable` だけを置くとファイル末尾まで検出が止まる点に注意する．

### punctuation への全角ピリオド追加

`MD036` の既定 `punctuation` は全角ピリオド `．`（U+FF0E）を含まない．
句点を `．` で統一する caller では，太字 1 文の段落がすべて誤検出される．
このため中央テンプレは既定の記号を残したまま `．` を追加している
（[Issue #102](https://github.com/tomio2480/github-workflows/issues/102)）．
