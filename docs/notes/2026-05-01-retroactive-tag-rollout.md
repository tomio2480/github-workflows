...
`gh release create --notes "$(cat <<EOF ... EOF)"` の heredoc 終端タグは無クォート扱いとなる．
そのため bash が `$` を展開する．
リテラル `$` を入れたい場合は終端タグを `'EOF'` のようにシングルクォートで囲むと展開を抑止できる．
ダブルクォート `"EOF"` はクォートなしと同じ扱いで展開が走る点も注意．
バッククォートはコードスパンを示すため Release ノート上でそのまま欲しい場合が多い．
今回は本文に `$GITHUB_ACTION_PATH` 等を書かなかったが，書く場合はシングルクォート版に切り替える運用とする．
