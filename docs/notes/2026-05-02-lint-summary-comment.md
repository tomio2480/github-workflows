...
契機は [Issue #24](https://github.com/tomio2480/github-workflows/issues/24) である．
reviewdog の `github-pr-review` reporter は findings ゼロのとき何も投稿しない．
PR を見たユーザーは「lint が走ったが指摘なし」と「workflow 未起動」を区別できなかった．
...
- 集計：[scripts/count-lint-findings.py](../../scripts/count-lint-findings.py) が担う．textlint の XML と `markdownlint` のテキストを読み，件数と findings 一覧を JSON にする
- 投稿：[scripts/post-lint-summary.sh](../../scripts/post-lint-summary.sh) が担う．JSON を Markdown 本文に rendering して PR コメントを upsert する
...
ただし利点が勝つ．bats で fake curl + JSON fixture のテストを書きやすくなるためである．
...
`integration-action-clean-only` のような job を追加し，「指摘ゼロ」ケースを別 job で目視確認する案があった．
当初の plan には含んでいたが，PR 着手中に race の懸念へ気付き drop した．
...
PR #28 では機械生成 Markdown 固有の sanitization 規律を観測した．
内容は改行・パイプ・絶対パス・フレームワーク内部 prefix の正規化である．
これは本リポジトリ単独では先回りしきれなかった観点である．
...
対象は `scripts/count-lint-findings.py` の `markdownlint` 行検出 regex である．
当初は `^.+:\d+(?::\d+)?\s+\S+/\S+`（greedy）だった．
