...
`markdown-lint` composite action の 3 連 Issue（#117・#118・#119）へ
連続で対応した．その際の設計判断と学びを記録する．`markdownlint` の
実行は `markdownlint-cli2` の 1 回へ一本化した（v2.11.0）．続けて
診断ログを足し（v2.11.1），summary へ検査対象 base を併記した
（v2.12.0）．
...
caller 実測で，2 経路の指摘件数が 870 対 61 で食い違った（#117）．
reviewdog 経路（`markdownlint-cli` v0.41.0）と summary 集計経路
（`markdownlint-cli2`）である．cli は cli2 形式設定の `config:` /
`ignores:` を解釈しないためである．また summary の全体件数が 0 になっても，
...
- **実行系は cli2 の 1 回へ統一する．** 集計用に走っていた cli2 の
  結果テキストを reviewdog CLI へ渡す．errorformat は列あり・列なしの
  2 つの `-efm` で受ける．設定解釈の不一致を構造ごと除去した．
...
- **summary へ検査対象 base を併記する．** 全体件数は checkout した
  マージコミットの base 時点の値である．main の現在値ではない．
...
- 実行失敗判定を「レポートが空か」で行う案（textlint ステップと
  同方式）: banner が必ず出てレポートが空にならず，流用できない．
...
- **遡及調査の限界．** 全出力を `> report.txt 2>&1 || true` で落とす
  ...
  診断ログの追加という予防で対応した．
- **副次発見．** 撤去前の `reviewdog/action-markdownlint` の挙動である．
  同じ run で，`markdownlint-cli` の引数解釈に失敗して usage を
  出力していた．
