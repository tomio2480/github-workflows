...
| 12 | 中央リポジトリ自身が comment 発火の workflow（`claude-review-self.yml`）を持ち，第三者のコメントで `pull-requests: write` を伴う job が起動しうる | 低 | `claude-code-action` が既定でコメント投稿者の write 権限を検査する．`allowed_non_write_users` は設定しない．`contents` は read に絞る（Issue [#140](https://github.com/tomio2480/github-workflows/issues/140)） |
| 13 | `pull_request_review_comment` 経由では PR 側の workflow 定義が実行され，checkout も PR の merge commit を取る | 低（write 権限者に限る） | 本イベントは default ブランチ限定の制約を受けない（Issue [#197](https://github.com/tomio2480/github-workflows/issues/197) で実測）．`GITHUB_SHA` が PR の merge commit を指すため，`ref` 未指定の checkout は default ブランチではなく PR の内容を取る．PR ブランチへ push できる者は workflow 自身を書き換えられ，投稿者の write 権限検査ごと外せる．**同一リポジトリの write collaborator は信頼境界の内側として扱う．** fork からの PR では repository secret が渡らず `GITHUB_TOKEN` も read-only へ降格するため，この経路は成立しない |
